...
The IV scorer compares the union of recurrent entities and uses zero when an
entity is absent from a continuation. Absence is not an estimated role state;
the role-transition probabilities and smoothing here are unchanged.
"""
...
from entity_roles import (INVENTORIES, deprel_states, mention_role as _role,  # noqa: E402
                          project_state)

ROLES = INVENTORIES["fine"]
ROLE_INVENTORY = "fine"
...
def role_of_mention(tokens: Sequence[dict], start: int, end: int) -> str:
    """Delegates to entity_roles so every script shares one definition."""
    return _role(tokens, mention_head(tokens, start, end), ROLE_INVENTORY)
...
def document_mentions(text: str) -> list[tuple[int, int, int, int, str]]:
    ...
    for sentence_index, tokens in enumerate(parse_conllu(text)):
        for start, end, entity_id in entity_spans(tokens):
            role = role_of_mention(tokens, start, end)
            if ROLE_INVENTORY == "deprel" and ROLES:
                role = project_state(role, ROLES)
            mentions.append((sentence_index, start, end, entity_id, role))
    return sorted(mentions, key=lambda item: (item[0], item[1], -item[2], item[3]))

def fit_deprel_states(path, min_label_count: int) -> list[str]:
    """For the deprel inventory the states are the labels themselves, so they
    have to be read off the corpus before the transitions are counted."""
    counts: Counter[str] = Counter()
    for _, text in iter_gum_documents(path):
        for _, _, _, _, role in document_mentions(text):
            counts[role] += 1
    states = deprel_states(counts, min_label_count)
    kept = sum(n for label, n in counts.items() if label in states)
    print(f"deprel inventory: {len(counts)} labels seen, {len(states)} states kept "
          f"(min count {min_label_count}), covering {100 * kept / sum(counts.values()):.1f}% "
          f"of mentions", flush=True)
    return states
...
def estimate(path: str | Path, smoothing: float) -> dict:
    ...
    return {
        ...
        "roles": list(ROLES), "role_inventory": ROLE_INVENTORY,
        "state_source": ("the Stanza/UD label of the mention-span head"
                         if ROLE_INVENTORY == "deprel" else "grouped roles"),
        "smoothing": smoothing,
        "cost_unit": "bits",
        ...
        "absence_convention": "IV scorer uses 0 for an entity absent from a continuation; absence is not a role in this table",
...
def main() -> None:
    ...
    parser.add_argument("--min-label-count", type=int, default=1,
                        help="deprel inventory only: labels rarer than this are folded "
                             "into 'other'. 1 keeps every label (exhaustive) but leaves "
                             "most of the transition matrix unobserved")
    args = parser.parse_args()
    global ROLES, ROLE_INVENTORY
    ROLE_INVENTORY = args.role_inventory
    ROLES = INVENTORIES[ROLE_INVENTORY]
    if ROLE_INVENTORY == "deprel":
        ROLES = tuple(fit_deprel_states(args.gum, args.min_label_count))
    if args.smoothing <= 0:
